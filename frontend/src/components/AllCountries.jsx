...
export default function AllCountries({ onClose, theme = "dark" }) {
...
  if (loading) {
    return (
      <div className={`p-4 text-center ${theme === "dark" ? "text-white" : "text-gray-800"}`}>
        Loading countries…
      </div>
    );
  }
...
        </button>
        <CountryDetails country={selectedCountry} theme={theme} />
      </div>
    );
  }
...
        className={`relative ${
          theme === "dark"
            ? "bg-gray-900 bg-opacity-60 backdrop-blur-xl text-white" 
            : "bg-white bg-opacity-90 backdrop-blur-xl text-gray-800 shadow-lg"
        } rounded-lg w-11/12 max-w-4xl mx-auto p-6 max-h-[80vh] overflow-y-auto`}
...
            } w-full sm:w-1/2 focus:outline-none`}
          />
          <select
...
                ? "bg-gray-800 text-white" 
                : "bg-gray-100 text-gray-800"
            } w-full sm:w-1/4 focus:outline-none`}
          >
